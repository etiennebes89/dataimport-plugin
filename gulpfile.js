...
const pluginOutputDir = "../dataimport-host/dataimport-plugin/dist";
...
function buildJs(src) {
  ...
  return gulp
      .src(src, { sourcemaps: true, since: gulp.lastRun(build) })
...
function build() {
  ...
  return (
      merge2(
          gulp.src("{src,dev-app}/**/*.json", { since: gulp.lastRun(build) }),
          gulp.src("{src,dev-app}/**/*.html", { since: gulp.lastRun(build) }),
          buildJs("{src,dev-app}/**/*.ts"),
          buildCss("{src,dev-app}/**/*.{scss,css}")
      )
...
function buildPlugin() {
  return (
      merge2(gulp.src("src/**/*.{json,html}"), buildJs("src/**/*.ts"), buildCss("src/**/*.{scss,css}"))
...
function clean() {
  return del([outputDir, pluginOutputDir], { force: true });
}
...
function watchPlugin() {
  gulp.watch("src/**/*", gulp.series(clean, buildPlugin));
}
...
exports["watch-plugin"] = watchPlugin;